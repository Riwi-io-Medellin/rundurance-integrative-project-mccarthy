  ...
  return new Promise((resolve, reject) => {
    ...
    parser.parse(buffer, (error, data) => {
      ...
      if (fileType === "workout") {
        return reject(
          new Error(
            "Este archivo es un plan de entrenamiento, no una actividad completada. Sube el archivo .FIT grabado por tu dispositivo Garmin después de correr.",
          ),
        );
      }
      ...
      if (!session)
        return reject(
          new Error(
            "No se encontró sesión en el archivo .FIT. Asegúrate de subir un archivo de actividad (no un plan de entrenamiento).",
          ),
        );
      ...
      const lapSource =
        (session.laps?.length ? session.laps : null) ?? data.laps ?? [];
